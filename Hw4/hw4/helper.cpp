...
int Helper::convertToint(char str) {
...
int Helper::convertToint(string str) {
    int basamak = 0;
    int sayi = 0;
    int state = +1;
...
int Helper::power(int base, int pow) {
    int result = 1;
    if (base == 0)
        return 0;
    for(int i = 0; i < pow; i++) {
...
bool Helper::controlCommands(string line) {
    int i = 0;
...
bool Helper::isonlyinteger(string line) {
    if (line[0] == '-') {
...
bool Helper::findComma(string line) {
    for(int i = 0; i < line.length(); i++) {
        if (line[i] == ',') 
            return true;
...
bool Helper::ControlSemicolon(string line) {
    bool Semicolon = true;
 ...
 string Helper::parseLine(string line, int *point) {
    string seperated = "";
    int a = *point;
 ...
 void Helper::convertToUpper(string& line) {
    for(int i = 0; i < line.length(); i++) {
...
bool Helper::firstLineControl(string line) {
...
bool Helper::isRegister(string command) {
...
bool Helper::isMemoryAddress(string line) {
