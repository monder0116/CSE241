...
void CPU::execute(string line, Memory& memArray) {
    ...
    if (!Helper::firstLineControl(line)) {
    ...
    int lastPoint = 0;
    bool work = false;
    command = Helper::parseLine(line, &lastPoint);
    Helper::convertToUpper(command);
    if (command == "PRN") {
        parm1 = Helper::parseLine(line, &lastPoint);
        Helper::convertToUpper(parm1);
        if (Helper::ControlSemicolon(&line[lastPoint])) {
            work = doPRN(parm1);
        }
        else
            work = false;
        ...
        Helper::convertToUpper(parm1);
        if (!Helper::ControlSemicolon(&line[lastPoint])) {
            parm2 = Helper::parseLine(line, &lastPoint);
            Helper::convertToUpper(parm2);
            if (Helper::ControlSemicolon(&line[lastPoint])) {
                if (command == "MOV")
...
bool CPU::doMOV(string parm1, string parm2) {
    ...
    if (parm1 == "" || parm2 == "")
        CommandWork = false;
    else if (Helper::isRegister(parm1) && Helper::isMemoryAddress(parm2)) {
        int parm1Num = Helper::convertToint(parm1);
        int parm2Num = Helper::convertToint(parm2);
        memories.setMem(parm2Num, getReg(parm1Num));

        CommandWork = true;

    }
    else if (Helper::isMemoryAddress(parm1) && Helper::isRegister(parm2)) {
        int parm1Num = Helper::convertToint(parm1);
        int parm2Num = Helper::convertToint(parm2);
    ...
    else if (Helper::isMemoryAddress(parm1) && Helper::isonlyinteger(parm2)) {
        int parm1Num = Helper::convertToint(parm1);
        int parm2Num = Helper::convertToint(parm2);
        if (parm2Num >= 0) {
    ...
    else if (Helper::isRegister(parm1) && Helper::isRegister(parm2)) {
        int parm1Num = Helper::convertToint(parm1[1]);
        int parm2Num = Helper::convertToint(parm2[1]);
        setReg(parm2Num, getReg(parm1Num));
        CommandWork = true;

    }
    else if (Helper::isRegister(parm1) && Helper::isonlyinteger(parm2)) {
        int constant = Helper::convertToint(parm2);
        int reg1Num = Helper::convertToint(parm1[1]);
...
bool CPU::doPRN(string parm1) {
    bool CommandWork = false;
    if (Helper::isRegister(parm1)) {
        int parm1Num = Helper::convertToint(parm1[1]);
        cout << getReg(parm1Num) << endl;
        CommandWork = true;
    }
    else if (Helper::isMemoryAddress(parm1)) {
        int parm1Num = Helper::convertToint(parm1);
...
bool CPU::doJMP(string parm1) {
    bool CommandWork = false;
    if (Helper::isonlyinteger(parm1)) {
        int linenumber = Helper::convertToint(parm1);
        if (linenumber > 0) {
            setPC(linenumber - 1);
            CommandWork = true;
...
bool CPU::doJMP(string parm1, string parm2) {
    bool CommandWork = false;
    if (Helper::isRegister(parm1) && Helper::isonlyinteger(parm2)) {
        int parm1Num = Helper::convertToint(parm1[1]);
        int parm2Num = Helper::convertToint(parm2);
        if (parm2Num > 0) {
